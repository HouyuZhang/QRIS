...
for (sample_prefix in prefixs){
  ...
  motifPrefix="./"
  motif <- read.table(paste0(motifPrefix,str_extract(sample_prefix,".*?_"),"motif_MEME_prob.txt"))
  ...
  fitted_model <- train(retro_cut ~ ., data = trainning_final,
  ...
  for(featureName in featureNames){
    ...
    for (trainning_plan in trainning_plans){
      ...
      if (trainning_plan == "shape"){ 
        trainning_final <- data.frame(retro_cut = bed$cut, featureVector)
      } else if (trainning_plan == "motif_shape"){
        trainning_final <- data.frame(retro_cut = bed$cut, bed[,6:56], featureVector)
        } 
      trainning_final$retro_cut <- factor(trainning_final$retro_cut, levels=c(0,1),labels = c("uncut","cut"))
